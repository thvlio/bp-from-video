...
class InferenceRunner:
    ...
    def __init__(self,
                 ...
                 *,
                 ...
                 roi_configs: tuple[str, list[int], tuple[float, float, float, float]] = c.SELECTED_ROI_CONFIGS) -> None:
    ...
    @profiler.timeit
    def run_pipe(self, frame: cv2.typing.MatLike, timestamp_ms: int | None = None, sort_largest: bool = True) -> list[c.Detections | c.Masks]:
    ...
    @profiler.timeit
    def calc_rois(self, inference_results: list[c.Detections | c.Masks]) -> list[c.Location]:
        _, (_, face_landmarks), (_, hand_landmarks), _ = inference_results
        rois = []
        for model_type, landmark_indices, (left_m, top_m, right_m, bottom_m) in self.roi_configs:
